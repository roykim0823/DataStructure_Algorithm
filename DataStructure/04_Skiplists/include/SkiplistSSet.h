...
typename SkiplistSSet<T>::Node* SkiplistSSet<T>::findPredNode(T x) {
	...
	while (r >= 0) {
		while (u->next[r] != NULL 
               && compare(u->next[r]->x, x) < 0)	// u->next[r]->x < x
...
bool SkiplistSSet<T>::add(T x) {
	...
	while (r >= 0) {
		while (u->next[r] != NULL 
			...
			u = u->next[r];
		if (u->next[r] != NULL && comp == 0)
			return false; 		// the x exists already
	...
	for (int i = 0; i < w->height; i++) {
		w->next[i] = stack[i]->next[i];	// new node points to next item
		stack[i]->next[i] = w;        	// prev node points to new node
	}
	n++;
	return true;
}
...
int SkiplistSSet<T>::pickHeight() {
	int z = rand();  	// tossing coins to deermine the height k 
	int k = 0;
	int m = 1;
	while ((z & m) != 0) {
		k++;
